...
const CheckoutScreen = () => {
  ...
  const [dateSelected, SelectedADate] = useState('');
  ...
  const timeSlots = () => {
    ...
    for (let i = startAM; i <= endAM; i++) {
      const label = `${i.toString().padStart(2, "0")}:00 AM ------ ${parseInt(1+i).toString().padStart(2, "0")}:00 AM`;
      const value = `${i}:00:00`;
      slots.push({ label, value });
    }
    for (let i = startPM; i <= endPM; i++) {
      const label = `${i.toString().padStart(2, "0")}:00 PM ------ ${parseInt(1+i).toString().padStart(2, "0")}:00 PM`;
  ...
  const handleDayPress = (day) => {
    console.log(day===selectedDate?'Date Match':'Not match')
    console.log(selectedDate+'-------------'+day)
    console.log('Date MatchNot match')
    SelectedADate(day)
    setSelectedDay(day);
  ...
  for (let i = 0; i < 7; i++) {
    ...
    weekday.setDate(today.getDay() + i);
  ...
  return (<>
    <ScrollView style={styles.container}>
      ...
      <View style={styles.form}>
        <Text style={{ fontWeight: 'bold', margin: 10 }}>Select Date : {JSON.stringify( dateSelected ==""?(new Date()).toString().substring(0,15): dateSelected.toString().substring(0,15))}</Text>
        <ScrollView
        ...
        <Text style={{ fontWeight: 'bold', margin: 10 }}>Select Time</Text>
